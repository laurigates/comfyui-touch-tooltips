...
function showPopover(x, y, label, sub, text) {
    ...
    let left = x + 14;
    let top = y + 14;
    if (left + rect.width > vw - 8) left = Math.max(8, x - rect.width - 14);
    if (top + rect.height > vh - 8) top = Math.max(8, y - rect.height - 14);
    if (left < 8) left = 8;
    if (top < 8) top = 8;
...
function widgetHeight(node, w) {
    ...
    return (lg && lg.NODE_WIDGET_HEIGHT) || 20;
}
...
function resolveTooltipForHit(node, hit) {
    if (hit.type === "widget") {
        const w = hit.widget;
        const fromOpts = w.options && typeof w.options.tooltip === "string"
            ? w.options.tooltip
            : null;
...
function attach() {
    ...
    el.addEventListener("pointermove", (e) => {
        ...
        if (Math.abs(e.clientX - startClientX) > MOVE_TOLERANCE_PX
    ...
    document.addEventListener("pointerdown", (e) => {
        ...
        if (!pop) return;
        if (!pop.contains(e.target)) dismissPopover();
    }, { capture: true });
